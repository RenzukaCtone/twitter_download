...
download_time_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S") # 开始下载时间，用于爬取的用户头像/banner文件名
# downloaded_profile_banner = set()
...
def get_other_info(_user_info):
    ...
    try:
        ...
        _user_info.profile_image_url_https = raw_data['data']['user']['result']['legacy']['profile_image_url_https'] if 'profile_image_url_https' in raw_data['data']['user']['result']['legacy'] else None
                ...
                if 'tweet' in i['entryId']:     #正常推文
                    ...
                    if _result[0]:  #符合时间限制
                        if 'retweeted_status_result' not in a : #判断是否为转推,以及是否获取转推
                            ...
                            profile_image_url_https = _user_info.profile_image_url_https
        ...
        async def down_save(url, prefix, csv_info, order: int):
            ...
            if not _profile_image and not _profile_image:
                ...
                if md_output: # 在下载完毕之前先输出到 Markdown，以尽可能保证高并发下载也能得到正确的推文顺序。
                    md_file.media_tweet_input(csv_info, prefix, downloaded_profile[csv_info[2]])
            count = 0
            while True:
                try:
